...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  appointmentTimeInput = document.getElementById("appointment-time"); // make sure this is a <select>
...
async function loadAvailableTimes() {
  ...
  if (!serviceId || !appointmentDate) {
    appointmentTimeInput.innerHTML = `<option disabled selected>Select date & service first</option>`;
    return;
  }
  ...
  try {
    ...
    const availableTimes = DEFAULT_TIME_SLOTS.filter(time => !bookedTimes.includes(time));

    if (availableTimes.length === 0) {
      appointmentTimeInput.innerHTML = `<option disabled selected>No times available</option>`;
      return;
    }
...
async function loadCategoriesAndServices() {
  ...
  categorySelect.addEventListener("change", () => {
    ...
    loadAvailableTimes(); // refresh available times after category change
  });
}
